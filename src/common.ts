...
export class Common implements ICommon {
	...
	isIOS() {
		...
		const isIOS = () => {
			const regex = new RegExp(/iPad|iPhone|iPod|iPad Simulator|iPhone Simulator|iPod Simulator/i);
	...
	jsonToHtml(obj: IJsonToHtml): HTMLElement {
		...
		for (let i in obj.children || []) {
			...
			if (
				(newElem && newElem.tagName && newElem.tagName.toLowerCase() !== 'undefined') ||
				newElem.nodeType === 3
			)
	...
	isFontLoaded(fontFamily?: string, callback?: Function) {
		try {
			const onReady = () => {
				return callback(
					(document as any).fonts.check(`1em ${(fontFamily as any).replaceAll('+', ' ')}`)
				);
