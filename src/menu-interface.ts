...
export class MenuInterface implements IMenuInterface {
	...
	invertColors(destroy: boolean) {
		...
		if (destroy) {
			this._acc.resetIfDefined(
				this._acc.stateValues.html.backgroundColor,
				this._acc.html.style,
				'backgroundColor'
			);
	...
	grayHues(destroy: boolean) {
		...
		if (destroy) {
			...
			this._acc.resetIfDefined(
				this._acc.stateValues.html.webkitFilter,
				this._acc.html.style,
				'webkitFilter'
			);
			this._acc.resetIfDefined(
				this._acc.stateValues.html.mozFilter,
				this._acc.html.style,
				'mozFilter'
			);
			this._acc.resetIfDefined(
	...
	speechToText(destroy: boolean) {
		...
		if (this._acc.stateValues.speechToText) {
			...
			let inputs = document.querySelectorAll(
				'input[type="text"], input[type="email"], input[type="tel"], input[type="search"], textarea, [contenteditable]'
			);
			for (let i = 0; i < inputs.length; i++) {
				inputs[i].addEventListener(
					...
					() => {
						if (
							typeof this._acc.recognition === 'object' &&
	...
	disableAnimations(destroy: boolean) {
		...
		allImages.forEach(async (i) => {
			...
			if (ext && ext.toLowerCase() === 'gif') {
				...
				if (!screenshot) screenshot = await this._acc.common.createScreenshot(i.src);
	...
	customFunctions(destroy?: boolean, button?: HTMLElement) {
		if (!button) return;
		const cf =
			this._acc.options.customFunctions[parseInt(button.getAttribute('data-access-custom-index'))];
